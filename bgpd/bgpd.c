...
static void bgp_load_vrf_rd_statefile(void)
{
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (sscanf(line, "%255s %u", namebuf, &rd) != 2) {
			zlog_warn("BGP: malformed line in %s: %s", BGP_VRF_RD_STATEFILE, line);
			continue;
		}
		if (rd == 0 || rd > UINT16_MAX) {
			zlog_warn("BGP: ignoring invalid rd_id %u for VRF %s in %s", rd, namebuf,
				  BGP_VRF_RD_STATEFILE);
			continue;
		}
...
static void bgp_load_vni_rd_statefile(void)
{
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (sscanf(line, "%u %u", &vni, &rd) != 2) {
			zlog_warn("BGP: malformed line in %s: %s", BGP_EVPN_VNI_RD_STATEFILE, line);
			continue;
		}
		if (rd == 0 || rd > UINT16_MAX) {
			zlog_warn("BGP: ignoring invalid rd_id %u for VNI %u in %s", rd, vni,
				  BGP_EVPN_VNI_RD_STATEFILE);
			continue;
		}
...
void bgp_free(struct bgp *bgp)
{
	...
	vrf_name = (bgp->inst_type == BGP_INSTANCE_TYPE_DEFAULT) ? "default"
								 : (bgp->name ? bgp->name : "?");
